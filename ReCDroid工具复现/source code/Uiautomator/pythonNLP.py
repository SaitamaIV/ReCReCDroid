...
#这段代码的功能是语句分割并且构建语法依赖树，确定单词的成分，确认词根
def main():
    #################xml part##################
    ########write##################
    '''
...
def pickupcrital(nlp,lineList, address):
    ...
    for sentence in lineList:        
        for sent in nlp(sentence).sents:
            ...
            ########all cases
            newsent=doc_allcase.createElement("Sent"+str(sentid))#识别事件
            ...
            nounlist=doc_allcase.createElement("nounlist")#创建名词列表
                    ...
                    if recorder.type=="input":
                        ...
                        if examplecase==True:               
                            if (word.pos_ in ["NUM"] and word.is_digit==False):
                                #print("test1")
                                #print(word.text)
                                recorder.digittypewhat.append(re.findall ('[\d ]+', word.lemma_))#数据
                                recorder.digittypewhere.append(re.findall ('[^\d ]+', word.lemma_))#字符串
            ...
            #########################click case####################
                if(word.lemma_ in ["click","choose","select","lanuch","pick","tap","open","press","go"]):
                    ...
                    ####8/8/2017 fix the if part，修复对条件状语从句的识别问题
            ...
            ###########################left out case#############################
                if(word.lemma_ in ["leave", "left"]):#区分语态
